...
class Combat {
  ...
  static updateRound(state, attacker) {
    ...
    //Logger.verbose(`${attacker.name} round started at ${lastRoundStarted}`);
    attacker.combatData.roundStarted = Date.now();

    // cancel if the attacker's combat lag hasn't expired yet
    //Logger.verbose(`${attacker.name} lag remaining: ${attacker.combatData.lag}`);
  ...
  static makeAttack(attacker, target) {
    ...
    attacker.combatData.message =  `<b><yellow>AS: ${AS} DS: ${DS} d100: ${d100} total: ${total}</yellow></b>`;
    ...
    Logger.verbose(`[Combat.js] Lag: ${attacker.combatData.lag}`);
