...
class Combat {
  ...
  static makeAttack(attacker, target, state) {
    ...
    //console.log('AS ' + AS + ' DS ' + DS + ' d100 ' + d100 + ' total ' + total)
    //Logger.info('AS ' + AS + ' DS ' + DS + ' d100 ' + d100 + ' total ' + total);
    ...
    if (total > 100) {
      ...
      //attacker.combatData.messageCrit = this.formatCritical(critDescription, attacker, target, state);
  ...
  static formatCritical(critDescription, attacker, target, state) {
    let descriptionArray = critDescription.split('.');
    let sentence;
    let message = '';

    for(sentence in descriptionArray) {
      console.log('sentence: ' + descriptionArray[sentence]);
      message += `<b><yellow>${descriptionArray[sentence]}</yellow></b>`;
      message += '\r\n';
    }
    console.log('descriptionArray: ' + descriptionArray);
    //let message = `<b><yellow>${critDescription}</yellow></b>`;
  ...
  static getWeaponDamageHits(attacker, total, state) {
    console.log('getWeaponDamageHIts total: ' + total);
    let weapon = this.findWeapon(attacker);
    // let weaponTable = Weapons.get(weapon.metadata.baseWeapon || 'weapon_broadsword');
  ...
  static calculateAttackStrength(attacker, weapon, state) {
    ...
    if(weapon?.metadata.weapon_type === undefined || weapon?.metadata.weapon_type === null) {
      weaponType = 'brawling';
      console.log('weapons.weaponType doesnt exist' + weaponType);
    }else{
      weaponType = weapon.metadata.weapon_type;
      console.log('weapons.weaponType exists' + weaponType);
    }
    let weaponSkill = attacker.getAttribute(weaponType);
    let strength = attacker.getAttribute('strength');
    let finalBonus = Math.round((weaponSkill + strength)*(stanceModifier.toFixed(2)/100));
    return finalBonus;
  }
